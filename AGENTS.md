...
云环境 ID：`cloud1-d5gbyvu3l05e11828`（前端 `app.ts` 和所有云函数均硬编码此值）

## 数据库关键约定

- 集合字段用 **snake_case**（`owner_openid`、`day_of_week`），前端类型用 **camelCase**（`ownerId`、`weekday`）
- `WeekDay`：1=周一 … 7=周日
- `FamilyRole`：`owner` | `edit` | `view`（家人共享固定同步为 `edit`，口令加入课表仍可能是 `view`）
- 账户级家人关系以 `families` 集合为准，`schedules.shared_with` 作为课表访问权限缓存
